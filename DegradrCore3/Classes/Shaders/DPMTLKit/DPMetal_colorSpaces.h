//
//  DPMetal_colorSpaces.h
//  DegradrCore3
//
//  Created by denis svinarchuk on 30.09.15.
//  Copyright © 2015 Degradr.Photo. All rights reserved.
//
//  Acknowledgement: http://easyrgb.com/
//
...
#ifndef __DPMETAL_COLOR_SPACES
#define __DPMETAL_COLOR_SPACES
...
METAL_FUNC  float rgb_2_L(float3 color)
{
...
METAL_FUNC float3 rgb_2_HSV(float3 c)
{
...
METAL_FUNC float3 HSV_2_rgb(float3 c)
{
...
METAL_FUNC float3 rgb_2_HSL(float3 color)
{
...
METAL_FUNC float hue_2_rgb(float f1, float f2, float hue)
{
    if (hue < 0.0)      hue += 1.0;
...
METAL_FUNC float3 HSL_2_rgb(float3 hsl)
{
    float3 rgb;
...
//
METAL_FUNC float3 rgb_2_XYZ(float3 rgb)
{
    float r = rgb.r;
    float g = rgb.g;
    float b = rgb.b;
...
METAL_FUNC float3 Lab_2_XYZ(float3 lab){
    ...
    xyz.x *= CIELAB_REF_X;    //     Observer= 2°, Illuminant= D65
    xyz.y *= CIELAB_REF_Y;
    xyz.z *= CIELAB_REF_Z;
...
METAL_FUNC float3 XYZ_2_rgb (float3 xyz){
...
METAL_FUNC float3 XYZ_2_Lab(float3 xyz)
{
    float var_X = xyz.x / CIELAB_REF_X;   //   Observer= 2°, Illuminant= D65
    float var_Y = xyz.y / CIELAB_REF_Y;
    float var_Z = xyz.z / CIELAB_REF_Z;
...
METAL_FUNC float3 Lab_2_rgb(float3 lab) {
    float3 xyz = Lab_2_XYZ(lab);
    return XYZ_2_rgb(xyz);
}

METAL_FUNC float3 rgb_2_Lab(float3 rgb) {
